...
class VehicleCardStore {
  ...
  deleteVehicle = (id) => {
    ...
    if (this.rootStore.mainStore.storeData.showingVehicles.length === 1) {
  ...
  editVehicle = (editedVehicle, id) => {
    ...
    this.rootStore.mainStore.vehicles = this.rootStore.mainStore.vehicles.map(
      (vehicle) => {
        if (editingVehicle.id === vehicle.id) {
          const result = mergeWith({}, vehicle, editingVehicle, (a, b) =>
            b === "" ? a : undefined
          );
          this.rootStore.mainStore.storeData.showingVehicles = [result];
          return result;
        }
        return vehicle;
      }
